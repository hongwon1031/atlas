...
- server의 hosting provider와 위치는 후속 결정입니다. [ADR-008](0008-initial-github-event-ingestion.md)의 polling-first trigger는 `Accepted`입니다. [ADR-009](0009-worker-process-supervision.md)의 tmux PoC와 stable supervisor 전환, [ADR-010](0010-task-execution-isolation.md)의 branch/worktree 격리는 `Accepted`이고 executor process 격리는 `Proposed`입니다.
